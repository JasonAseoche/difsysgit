...
const EADetails = ({ userLevel }) => {
  ...
  useEffect(() => {
    document.title = "DIFSYS | MY DETAILS";
  }, []);
  ...
  const fetchEmployeeData = async () => {
    try {
      ...
      console.log('Basic Info from response:', response.data.basicInfo);
      console.log('Profile Image from basicInfo:', response.data.basicInfo?.profileImage);
      ...
      if (response.data.error) {
        ...
        const basicInfoData = {
          ...(response.data.basicInfo || basicInfo),
          profileImage: response.data.basicInfo?.profileImage || response.data.profileImage || response.data.profile_image || ''
        };
        
        console.log('Final basicInfo profileImage:', basicInfoData.profileImage);
  ...
  const handleSaveClick = async () => {
    try {
      ...
      setError('Failed to save profile details: ' + (err.response?.data?.error || err.message));
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="eaprof-basic-content">
          <div className="eaprof-left-section">
            ...
            <div className="eaprof-profile-details">
              ...
              <p className="eaprof-id">{basicInfo.id || 'N/A'}</p>
              <p className="eaprof-gender">{basicInfo.gender || 'N/A'}</p>
              <p className="eaprof-email">{basicInfo.email || 'N/A'}</p>
              <p className="eaprof-contact">{basicInfo.contact || 'N/A'}</p>
            </div>
          </div>
          ...
          <div className="eaprof-right-section">
            {isEditing ? (
                ...
                <div className="eaprof-info-row">
                    <span className="eaprof-info-label">Position</span>
                    <input
                    type="text"
